...
scale = 4.0
x = f1 * scale
y = f2 * scale  # U JS je ovo Z os (f2)
z = f3 * scale + 2.0  # U JS je ovo Y os (f3 + 2)
...
# --- PLOTTER 2 ---
# plotter = pv.Plotter(lighting=None)  # Ovo je "čišći" način za ugasiti sve
# plotter.add_light(pv.Light(light_type="headlight", intensity=0.6))
...
for light in plotter.renderer.lights:
    light.intensity = 0.6  # prigušenje
...
sh_floor_actor = plotter.add_mesh(proj_z, **proj_args)

# Sjena na ZADNJI ZID
proj_y = grid_surface.copy()
proj_y.points[:, 1] = wall_offset - 0.01
sh_back_actor = plotter.add_mesh(proj_y, **proj_args)

# Sjena na LIJEVI ZID
proj_x = grid_surface.copy()
proj_x.points[:, 0] = -wall_offset + 0.01
sh_left_actor = plotter.add_mesh(proj_x, **proj_args)
...
# --- 5. FUNKCIJA ZA ROTACIJU I SJENE ---
# 'obj' i 'event' su obavezni parametri za ovaj tip callbacka
def rotate_callback(obj, event):
    # 1. Rotiraj bazičnu geometriju
    grid_surface.rotate_z(0.4, inplace=True)

    # 2. Ažuriraj zlato i mrežu
    gold_actor.mapper.dataset.points = grid_surface.points
    mesh_actor.mapper.dataset.points = grid_surface.points

    # 3. Ažuriraj SJENE (Pod, Zidovi) koristeći tvoj wall_offset
    pts = grid_surface.points
    sh_floor_actor.mapper.dataset.points = np.c_[pts[:, :2], np.full(len(pts), 0.01)]
    sh_back_actor.mapper.dataset.points = np.c_[
        pts[:, 0], np.full(len(pts), 4.99), pts[:, 2]
    ]
    sh_left_actor.mapper.dataset.points = np.c_[
        np.full(len(pts), -4.99), pts[:, 1], pts[:, 2]
    ]

    plotter.render()


# --- 6. POKRETANJE (Popravljeno da radi SVUGDJE) ---

# Umjesto add_callback ili timer_eventa, koristimo direktni interaktor
# Ovo je najstabilniji način: Q i X će raditi 100%
plotter.iren.add_observer("TimerEvent", rotate_callback)
plotter.iren.create_timer(15)  # 15ms interval

# HDR FIX: Ako enable_environment_lighting ne postoji, ovo je alternativa
# koja ručno pali Image Based Lighting na rendereru
try:
    plotter.renderer.GetEnvMapLookupTable()  # inicijalizacija
    plotter.renderer.UseImageBasedLightingOn()
except Exception:
    pass  # Ako i ovo padne, bar se kod neće srušiti

plotter.camera_position = [(16, -18, 12), (0, 0, 4), (0, 0, 1)]
plotter.camera.zoom(0.8)

print("Sve sjene se rotiraju. 'q' ili 'x' za izlaz.")
plotter.show()
